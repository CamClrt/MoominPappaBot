...
from config import FORMULATION
...
class Parser:
    ...
    def extract_location(self, sentence):
        """Parse sentence and extract location: to use this method suposed that
        you have already used the 3 methods:
        ...
        for formulation in self.formulation:
            formulation_without_case_sensitive = self.remove_case_sensitive(
                formulation
            )
            formulation_without_accents = self.remove_accents(
                formulation_without_case_sensitive
            )
            formulation_without_ponctuation = self.remove_ponctuation(
                formulation_without_accents
            )
            ...
            if re.search(normelized_formulation_regex, sentence):
                ...
                location_with_stop_words = extract.group().replace(
                    normelized_formulation, ""
                )
                location_without_stop_words = self.remove_stop_words(
                    location_with_stop_words
                )
